...
class Solution {
public:
    vector<vector<string>> accountsMerges(vector<vector<string>>& accounts) {
    ...
    vector<vector<string>> accountsMerge(vector<vector<string>>& accounts) {
        ...
        unordered_map<int, vector<string>> owners;
        for (auto& [email, id] : emailToAccount) {
            int parent = ds.findParent(id);
            owners[parent].push_back(email);
        }

        vector<vector<string>> re;
        for (auto& [id, emails] : owners) {
